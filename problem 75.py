...
limit = 1500000
length_frequencies = [0] * (limit + 1)

for n in range(1, 1000):  # ONE THOUSAND IS AN ARBITRARY LIMIT, I COULDN'T BE BOTHERED TO CALCULATE IT
    for m in range(n + 1, 1000, 2):
        ...
        if length <= limit:
            for k in range(1, limit // length + 1):
                length_frequencies[k * length] += 1

print(length_frequencies.count(1))
